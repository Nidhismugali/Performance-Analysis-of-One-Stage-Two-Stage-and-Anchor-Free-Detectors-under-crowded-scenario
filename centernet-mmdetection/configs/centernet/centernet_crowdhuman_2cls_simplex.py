...
#
# IMPORTANT:
#   - This file must be copied as-is. There should be NO `read_base`
#     and NO `with ...:` blocks anywhere in this file.
...
def _build_ann_file_for_split(data_root: str, split_choice: str) -> str:
    ...
    We support these options *if* you have already created:
        data/crowdhuman_coco/annotations/train01.json
        data/crowdhuman_coco/annotations/train02.json
        data/crowdhuman_coco/annotations/train03.json

    Otherwise we silently fall back to the original train.json.
    """
    ...
    # the robust way is to merge them offline into a single JSON.
    # To keep the config simple, we just fall back to the full train.json
    # when multiple files would be needed.
    if len(files) != 1:
        return base_ann_rel.replace("\\", "/")
...
def _enumerate_checkpoints(exp_dir: str) -> List[str]:
    """Return sorted list of epoch_*.pth inside exp_dir."""
    if not os.path.isdir(exp_dir):
        return []
    names = [n for n in os.listdir(exp_dir)
...
def _pick_checkpoint() -> Optional[str]:
    ...
    Returns:
        - absolute path to .pth
        - or None (for training from scratch)
    """
    print()
    ...
    choice = _safe_input_choice("Enter choice [default 0]: ",
                                ["0", "1", "2", "3"], "0")
    ...
    idx = _safe_input_int(
        f"Select checkpoint index [0-{len(ckpts)-1}, default {len(ckpts)-1}]: ",
        default=len(ckpts)-1,
        min_val=0,
    )
    if idx >= len(ckpts):
...
def _build_work_dir(load_from: Optional[str]) -> str:
    ...
    - If training from scratch: use default simple dir.
    - If loading from some exp dir: create a new subdir with suffix "_finetune".
    """
    base_simple = "work_dirs/centernet_crowdhuman_2cls_simple"
    if not load_from:
        return base_simple

    # e.g., load_from = /.../work_dirs/centernet_crowdhuman_2cls_strongmix/epoch_9.pth
    parent = os.path.dirname(os.path.dirname(os.path.abspath(load_from)))  # /.../work_dirs
    src_exp = os.path.basename(os.path.dirname(os.path.abspath(load_from)))  # exp folder name
    new_name = src_exp + "_finetune"
    candidate = os.path.join(parent, new_name)
    return candidate

# ======== INTERACTIVE PROMPTS (executed when config is loaded) ========
...
# 3) Checkpoint to start from
_load_from_ckpt = _pick_checkpoint()

# 4) Work dir (based on checkpoint choice)
data_root = "data/"
...
# ======== STANDARD MMDET CONFIG (static dictionaries from here on) ========
...
train_pipeline = [
    ...
    # Simple deterministic resize (no random augmentation).
    # If you REALLY want completely raw resolution, you can comment this out,
    # but then GPU memory / tensor size might explode.
...
# Keep ALL checkpoints by default so earlier epochs are not deleted.
default_hooks = dict(
...
# Important: this tells MMEngine which weights to load (if any).
